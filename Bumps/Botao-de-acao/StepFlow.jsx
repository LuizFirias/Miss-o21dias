...
const stepTransition = { duration: 0.22, ease: [0.32, 0.72, 0, 1] };

/* ─────────────────────────────────────────────
   SHARED ATOMS
───────────────────────────────────────────── */
function Tag({ children, color = "#FF3B3B" }) {
  return (
    <motion.span
...
function BigText({ children, color = "#F5F5F5", size = "56px", delay = 0.1 }) {
  return (
    <motion.h1
...
function SubText({ children, delay = 0.18 }) {
  return (
    <motion.p
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
...
function RedButton({ children, onClick, delay = 0.22, outline = false }) {
  return (
    <motion.button
      onClick={onClick}
...
function StepDone({ onFinish }) {
  return (
    ...
    >
      ...
      <div style={{ height: "48px" }} />
      <RedButton onClick={onFinish} delay={0.32} outline>VOLTAR PARA MISSÕES</RedButton>
    </div>
  );
}
...
───────────────────────────────────────────── */
export default function StepFlow({ onFinish, onStepChange }) {
